...
initial_state_ego = [0, 0, 0, 10, 0, 0, 0, 0]
initial_state_obstacle = [10.0, 0, 0, 10, 2.0, 0, 0, 0.0]
...
def HeuristicSearch(
    ...
    if behavior_state == longitudinal_yield:
        safe_distance = 2.0
        sf_ref = st_graph[nums_of_knots - 1] - safe_distance
    ...
    s_ref = ds_ref * planning_horizon
    l_ref = dl_ref * planning_horizon
    ...
    goal_state = VehicleState(
        s_ref, ds_ref, dds_ref, l_ref, dl_ref, ddl_ref, planning_horizon
    )
    goal_node = Node(goal_state)
    final_node = Node(goal_state)
    ...
    while open_set:
        ...
        for dds_next_node in derivate_space.a_candidate_set:
            tranverse_ind = derivate_space.GetTranverseInd(
                current_node.longitudinal_state.dds, dds_next_node
            )
            tranverse_dds = derivate_space.a_tranverse_pairs[tranverse_ind[0]][
                tranverse_ind[1]
            ]
            tranverse_ds = derivate_space.ds_tranverse_pairs[tranverse_ind[0]][
                tranverse_ind[1]
            ]
            tranverse_s = derivate_space.s_tranverse_pairs[tranverse_ind[0]][
                tranverse_ind[1]
            ]
            tranverse_cost = derivate_space.transition_cost_pairs[tranverse_ind[0]][
                tranverse_ind[1]
            ]
            tranverse_state = TranverseState(
                tranverse_s, tranverse_ds, tranverse_dds, tranverse_cost
            )
...
def PlanningSimulate(simulate_steps, simulate_delta_t):
    ...
    # derivate_space.TraversePairsInit()
    dds_ego = 0.0
    ddl_ego = 0.0
    for i in range(0, simulate_steps):
        ...
        s_ref, ds_ref, dds_ref = HeuristicSearch(
            derivate_space,
            current_state_ego,
            dds_ego,
            ddl_ego,
        ...
        current_state_obstacle[0] = (
            last_state_obstacle[0]
            + last_state_obstacle[3]
            * math.cos(last_state_obstacle[2])
            * simulate_delta_t
        )
        current_state_obstacle[1] = (
            last_state_obstacle[1]
            + last_state_obstacle[3]
            * math.sin(last_state_obstacle[2])
            * simulate_delta_t
        )
        current_state_obstacle[2] = 0
        current_state_obstacle[3] = (
            last_state_obstacle[3] - obstacle_acc * simulate_delta_t
        )
        # to avoid yinyong
        simulate_states_ego.append(
            [
                current_state_ego[0],
                current_state_ego[1],
                current_state_ego[2],
        ...
        simulate_states_obstacle.append(
            [
                current_state_obstacle[0],
                current_state_obstacle[1],
                current_state_obstacle[2],
                current_state_obstacle[3],
            ]
        )
...
def Update(frame_number):
    ...
    ax.add_patch(
        patches.Rectangle(
            (
                current_state_ego[0] - vehicle_length / 2,
                current_state_ego[1] - vehicle_width / 2,
            ),
            vehicle_length,
            vehicle_width,
            angle=current_state_ego[2],
            edgecolor="green",
            linewidth=1.0,
            facecolor="green",
            fill=True,
    ...
    ax.add_patch(
        patches.Rectangle(
            (
                current_state_obstacle[0] - vehicle_length / 2,
                current_state_obstacle[1] - vehicle_width / 2,
            ),
            vehicle_length,
            vehicle_width,
            angle=current_state_obstacle[2],
            edgecolor="blue",
            facecolor="blue",
            fill=True,
        )
...
animation = FuncAnimation(
    fig, Update, frames=animation_frames, interval=animation_delta_t * 1000
)
