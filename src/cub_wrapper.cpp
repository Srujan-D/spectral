...
extern "C" {
	...
	{
		double acc;
		ifstream ifs("/home/srujan_d/RISS/code/btrapz/src/c_road_s1_3.txt");
		// ifstream ifs(path);
		...
		for (int i = 0; i < num_of_points && is_verbose; ++i) {
			...
			s_cost += (s[i] - x_ref[i]) * (s[i] - x_ref[i]) * delta_t;
			s_cost += ds[i] * ds[i] * delta_t;
		...
		for (int i = 0; i < num_of_points && is_verbose; ++i) {
			...
			l_cost += (l[i] - y_ref[i]) * (l[i] - y_ref[i]) * delta_t;
			l_cost += dl[i] * dl[i] * delta_t;
			l_cost += ddl[i] * ddl[i] * delta_t;
			l_cost += dddl * dddl * delta_t;
		...
		std::string file = "/home/srujan_d/RISS/code/btrapz/src/s1_cub_3d_";
