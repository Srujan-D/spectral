...
namespace navigation
{
	...
	void PiecewiseJerkSpeedProblem::CalculateOffset(std::vector<c_float> *q)
	{
		...
		// for(int i=0; i < (*q).size(); i++)	cout<<"q["<<i<<"] = "<<q->at(i)<<"\n";
		p_shift = 0;
		ref_ind = 0;
		...
		for (int k = 0; k < segment_num; k++)
		{
			...
			for (int i = 0; i < n_poly; i++)
			{
				...
				{	
					...
					// q_p(0, i) += -2.0 * pow(new_corridor[k].t, 2) * weight_y_ref_ * y_ref_[i + n_poly * k] / (i + 1);
					
					// ref_ind = int(1 + (i+n_poly*k)*double(num_of_knots_/double(segment_num*n_poly)));
					// q_p(0, i) += -2.0 * pow(new_corridor[k].t, 2) * weight_y_ref_ * y_ref_[ref_ind] / (i + 1);
					
					// cout<<"y_ref is \t"<<weight_y_ref_<<"\t"<<y_ref_[ref_ind]<<"\t"<<y_ref_[i + n_poly * k]<<"\n";
				}
	...
	void PiecewiseJerkSpeedProblem::CorridorGeneration()
	{
		...
		int j = 0;
		{
			...
			mcube.l_down_skew = (y_bounds_[1].first - y_bounds_[0].first) / delta_s_;
			mcube.l_down_bias = y_bounds_[0].first;
			mcube.l_upp_skew = (y_bounds_[1].second - y_bounds_[0].second) / delta_s_;
			mcube.l_upp_bias = y_bounds_[0].second;
		...
		for (int i = 2; i < n - 1; i++)
		{
			...
			double l_uskew, l_dskew;

			l_dskew = (y_bounds_[i].first - y_bounds_[i - 1].first) / delta_s_;
			l_uskew = (y_bounds_[i].second - y_bounds_[i - 1].second) / delta_s_;

			mcube.l_down_bias = y_bounds_[i].first;
			mcube.l_upp_bias = y_bounds_[i].second;

			mcube.l_down_skew = l_dskew;
			mcube.l_upp_skew = l_uskew;
		...
		// 	if ((abs(l_dskew - corridor[j - 1].l_down_skew) > mthre) || (abs(l_uskew - corridor[j - 1].l_upp_skew) > mthre))
		// 	{
		// 		corridor[j - 1].end_t = i;

		// 		mcube.beg_t = i;

		// 		mcube.l_down_skew = (y_bounds_[i + 1].first - y_bounds_[i].first) / delta_s_;
		// 		//mcube.down_bias = 2 * y_bounds_[i].first - y_bounds_[i + 1].first;
		// 		mcube.down_bias = y_bounds_[i].first;

		// 		mcube.l_upp_skew = (y_bounds_[i + 1].second - y_bounds_[i].second) / delta_s_;
		// 		//mcube.upp_bias = 2 * y_bounds_[i].second - y_bounds_[i + 1].second;
		// 		mcube.upp_bias = y_bounds_[i].second;
	...
	void PiecewiseJerkSpeedProblem::CollisionCheck()
	{
		...
		std::sort(temp.begin(), temp.end(), [](const Cube & r1,const Cube & r2) -> bool {return r1.beg_t < r2.beg_t; });

		// cout<<"preliminary sorting with time \n\n";
		// for (int i = 0; i < temp.size(); i++)
		...
		for (int i = 0; i < temp.size() - 1; i++)
		{	
			start_corridor = temp[i];
			for (int j = i + 1; j < temp.size(); j++)
			{	
				if (temp[i].beg_l == temp[j].beg_l && j-i == 1)
				{	
					// cout<<"continuity in corridors\ti = "<<i<<"\tj = "<<j<<"\n";
					// i++;
					break;
				}
				else{
					// if (temp[i].end_l == temp[j].beg_l){
						// cout<<"break in corridors\ti = "<<i<<"\tj = "<<j<<"\n";
						for (int k = j + 1; k < temp.size(); k++){
							// cout<<"evaluating next corridors\n";
							if (temp[i].beg_l == temp[k].beg_l && temp[i].end_t == temp[k].beg_t){
								// cout<<"\nswapping\ti = "<<i<<"\tj = "<<j<<"\tk = "<<k<<"\n";
								// for (int iter = 0; iter < temp.size(); iter++)	std::cout << temp[iter].beg_t << " " << temp[iter].end_t << " " << temp[iter].down_bias << " " << temp[iter].upp_bias << " " << temp[iter].down_bias + temp[iter].down_skew * delta_s_ << " " << temp[iter].upp_skew * delta_s_ + temp[iter].upp_bias << " " << temp[iter].beg_l << " " << temp[iter].end_l << "\n";
		...
		for (int i = 0; i < temp.size() - 1; i++)
		{
			...
			{	
				...
				{
					int diff = (temp[i].end_t - temp[i].beg_t) / 2;
				...
				else if (temp[i].beg_t > temp[j].beg_t && temp[i].end_t <= temp[j].end_t){
					int diff = (temp[i].end_t - temp[i].beg_t) / 2;
					if (diff > 1){
						temp[i].end_t = temp[i].end_t - (diff);
						temp[i].t = (temp[i].end_t - temp[i].beg_t) * delta_s_;
	...
	void PiecewiseJerkSpeedProblem::CorridorSplit()
	{
		...
		for (int k = 0; k < temp_num; k++)
		{
			while (corridor[k].t > 1)
			{
				...
				mcube.l_down_skew = corridor[k].l_down_skew;
				mcube.l_down_bias = corridor[k].l_down_bias;
				...
				mcube.l_upp_skew = corridor[k].l_upp_skew;
				mcube.l_upp_bias = corridor[k].l_upp_bias;
		...
		std::vector<c_float> *upper_bounds)
	{
		...
		for (int k = 0; k < segment_num; k++) // for each segment
		{
			...
			for (int i = 0; i < n_poly - 1; i++) // for 1st order trajectory (velocity)
			{
				...
				// cout<<"dx constrtaint lower\t"<<lower_bounds->at(constraint_index)<<"\tupper\t"<<upper_bounds->at(constraint_index)<<"\n";
				++constraint_index;
			}
			...
			for (int i = 0; i < n_poly - 2; i++) // for 2nd order trajectory (acc)
			{
				...
				// cout<<"ddx constrtaint lower\t"<<lower_bounds->at(constraint_index)<<"\tupper\t"<<upper_bounds->at(constraint_index)<<"\n";
		...
		for (int k = 0; k < segment_num; k++) // for each segment
		{
			...
			for (int i = 0; i < n_poly; i++) // bounds for each control point
			{
				...
				lower_bounds->at(constraint_index) = new_corridor[k].l_down_bias + new_corridor[k].l_down_skew * inv_M(i, 1) * new_corridor[k].t;
				upper_bounds->at(constraint_index) = new_corridor[k].l_upp_bias + new_corridor[k].l_upp_skew * inv_M(i, 1) * new_corridor[k].t;
				...
				// upper_bounds->at(constraint_index) = new_corridor[k].end_l+1e-3;
			...
			// PHYSICAL CONSTRAINTS :
			// cout << "\n\n-----------------physical constraints----------------------\n\n";
			double dy_lower_bound = -100.0, dy_upper_bound = 100;
			double ddy_lower_bound = -100, ddy_upper_bound = 100;
			...
			for (int i = 0; i < n_poly - 1; i++) // for 1st order trajectory (velocity)
			{
				...
				// cout<<"dy lower bound is : "<< lower_bounds->at(constraint_index)<<"\t and upper bound is: "<<upper_bounds->at(constraint_index)<<"\n";
				++constraint_index;
			}
			...
			for (int i = 0; i < n_poly - 2; i++) // for 2nd order trajectory (acc)
			{
				...
				// cout<<"ddy lower bound is : "<< lower_bounds->at(constraint_index)<<"\t and upper bound is: "<<upper_bounds->at(constraint_index)<<"\n";
				++constraint_index;
			}
			...
			for (int i = 0; i < n_poly - 3; i++) // for 3rd order trajectory (jerk)
			{
				...
				// cout<<"dddy lower bound is : "<< lower_bounds->at(constraint_index)<<"\t and upper bound is: "<<upper_bounds->at(constraint_index)<<"\n";
				++constraint_index;
			}
	...
	OSQPData *PiecewiseJerkSpeedProblem::FormulateProblem()
	{
		...
		for (int k = 0; k < segment_num; k++)
		{
			...
			// cout<<"y_bias \t"<<y_bias_[k]<<"\ty_skew \t"<<y_skew_[k]<<"\n";
		}
	...
	bool PiecewiseJerkSpeedProblem::Optimize(const int max_iter)
	{
		...
		else if (osqp_work->info->obj_val != osqp_work->info->obj_val)
		{
			// nan returned in solution
			osqp_cleanup(osqp_work);
			FreeData(data);
			c_free(settings);
			return false;
