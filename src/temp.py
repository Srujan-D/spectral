...
def write_bounds(bounds, ego):
	corridor_file = open(
		"/home/srujan_d/RISS/code/btrapz/src/c_road_s1_2.txt", 'w')
...
def write_reference_traj(corridor_file):
	...
	input_txt = "/home/srujan_d/RISS/code/btrapz/src/s1_slt_3d_201.txt"
	f = open(input_txt)
	ref_y = []
	ref_x = []
...
ego_vehicle_init_state = State(position=np.array([ego_y[0]+3.0, ego_x[0]]),
							   velocity=(ego_dx[0]**2 + ego_dy[0]**2)**0.5,
							   orientation=0.0,
							   time_step=0)
...
state_list = []
for i in range(1, 2):
	new_pos = np.array([ego_y[i]+3.0, ego_x[i]])
	new_state = State(position=new_pos, velocity=(
		ego_dx[i]**2 + ego_dy[i]**2)**0.5, orientation=angle[i], time_step=i)
	state_list.append(new_state)

ego_vehicle_traj = Trajectory(1, state_list)
ego_vehicle_shape = Rectangle(length=1.0, width=0.10)
...
dt = scenario.dt
vehicle_dynamics = VehicleDynamics.PM(vehicle_type=VehicleType.VW_VANAGON)
feasible, reconstructed_inputs = feasibility_checker.trajectory_feasibility(ego_vehicle_traj, vehicle_dynamics, dt)
